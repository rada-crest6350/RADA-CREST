// RADA CREST Pan-India Multi-Language Manager
const translations = {
  en: {
    store_title: "RADA CREST Store",
    help_title: "Help & Support",
    notif_title: "Activity Center",
    orders_title: "My Orders",
    buy_now: "Buy Now",
    track_req: "Track My Requests",
    notifications: "Notifications",
    wishlist: "Wishlist",
    address_title: "Delivery Address"
  },
  hi: { // Hindi
    store_title: "राडा क्रెస్ट स्टोर",
    help_title: "सहायता और समर्थन",
    notif_title: "गतिविधि केंद्र",
    orders_title: "मेरे ऑर्डर",
    buy_now: "अभी खरीदें",
    track_req: "मेरे अनुरोध को ट्रैक करें",
    notifications: "सूचनाएं",
    wishlist: "विशलिस्ट",
    address_title: "डिलीवरी का पता"
  },
  te: { // Telugu
    store_title: "రాడా క్రెస్ట్ స్టోర్",
    help_title: "సహాయం & మద్దతు",
    notif_title: "యాక్టివిటీ సెంటర్",
    orders_title: "నా ఆర్డర్లు",
    buy_now: "ఇప్పుడే కొనండి",
    track_req: "నా అభ్యర్థనలను ట్రాక్ చేయండి",
    notifications: "నోటిఫికేషన్‌లు",
    wishlist: "విష్‌లిస్ట్",
    address_title: "డెలివరీ చిరునామా"
  },
  ta: { // Tamil
    store_title: "ராடா கிரெஸ்ட் ஸ்டோர்",
    help_title: "உதவி & ஆதரவு",
    notif_title: "செயல்பாட்டு மையம்",
    orders_title: "எனது ஆர்டர்கள்",
    buy_now: "இப்போது வாங்கு",
    track_req: "எனது கோரிக்கையை கண்காணிக்கவும்",
    notifications: "அறிவிப்புகள்",
    wishlist: "விருப்பப் பட்டியல்",
    address_title: "டெலிவரி முகவரி"
  },
  kn: { // Kannada
    store_title: "ರಾಡಾ ಕ್ರೆಸ್ಟ್ ಸ್ಟೋರ್",
    help_title: "ಸಹಾಯ ಮತ್ತು ಬೆಂಬಲ",
    notif_title: "ಚಟುವಟಿಕೆ ಕೇಂದ್ರ",
    orders_title: "ನನ್ನ ಆರ್ಡರ್‌ಗಳು",
    buy_now: "ಈಗ ಖರೀದಿಸಿ",
    track_req: "ನನ್ನ ವಿನಂತಿಯನ್ನು ಟ್ರ್ಯಾಕ್ ಮಾಡಿ",
    notifications: "ಅಧಿಸೂಚನೆಗಳು",
    wishlist: "ವಿಶ್‌ಲಿಸ್ಟ್",
    address_title: "ವಿಳಾಸ"
  },
  ml: { // Malayalam
    store_title: "രാഡ ക്രെസ്റ്റ് സ്റ്റോർ",
    help_title: "സഹായം & പിന്തുണ",
    notif_title: "ആക്ടിവിറ്റി സെന്റർ",
    orders_title: "എന്റെ ഓർഡറുകൾ",
    buy_now: "ഇപ്പോൾ വാങ്ങുക",
    track_req: "അഭ്യർത്ഥന ട്രാക്ക് ചെയ്യുക",
    notifications: "അറിയിപ്പുകൾ",
    wishlist: "വിഷ്‌ലിസ്റ്റ്",
    address_title: "വിലാസം"
  },
  mr: { // Marathi
    store_title: "राडा क्रेस्ट स्टोअर",
    help_title: "मदत आणि समर्थन",
    notif_title: "ॲक्टिव्हिटी सेंटर",
    orders_title: "माझ्या ऑर्डर्स",
    buy_now: "आता खरेदी करा",
    track_req: "विनंती ट्रॅक करा",
    notifications: "सूचना",
    wishlist: "विशलिस्ट",
    address_title: "पत्ता"
  },
  gu: { // Gujarati
    store_title: "રાડા ક્રસ્ટ સ્ટોર",
    help_title: "મદદ અને સપોર્ટ",
    notif_title: "એક્ટિવિટી સેન્ટર",
    orders_title: "મારા ઓર્ડર",
    buy_now: "હમણાં ખરીદો",
    track_req: "વિનંતી ટ્રેક કરો",
    notifications: "સૂચનાઓ",
    wishlist: "વિશલિસ્ટ",
    address_title: "સરનામું"
  },
  bn: { // Bengali
    store_title: "রাডা ক্রুস্ট স্টোর",
    help_title: "সহায়তা এবং সমর্থন",
    notif_title: "অ্যাক্টিভিটি সেন্টার",
    orders_title: "আমার অর্ডার",
    buy_now: "এখনই কিনুন",
    track_req: "অনুরোধ ট্র্যাক করুন",
    notifications: "বিজ্ঞপ্তি",
    wishlist: "উইশলিস্ট",
    address_title: "ঠিকানা"
  },
  pa: { // Punjabi
    store_title: "ਰਾਡਾ ਕਰੈਸਟ ਸਟੋਰ",
    help_title: "ਮਦद ਅਤੇ ਸਹਾਇਤਾ",
    notif_title: "ਸਰਗਰਮੀ ਕੇਂਦਰ",
    orders_title: "ਮੇਰੇ ਆਰਡਰ",
    buy_now: "ਹੁਣੇ ਖਰੀਦੋ",
    track_req: "ਬੇਨਤੀ ਟ੍ਰੈਕ ਕਰੋ",
    notifications: "ਸੂਚਨਾਵਾਂ",
    wishlist: "ਵਿਸ਼ਲਿਸਟ",
    address_title: "ਪਤਾ"
  }
};

function setLanguage(lang) {
  localStorage.setItem('rc_lang', lang);
  applyTranslations();
}

function getLanguage() {
  return localStorage.getItem('rc_lang') || 'en';
}

function applyTranslations() {
  const lang = getLanguage();
  const elements = document.querySelectorAll('[data-i18n]');
  
  elements.forEach(el => {
    const key = el.getAttribute('data-i18n');
    if (translations[lang] && translations[lang][key]) {
      if (el.tagName === 'INPUT' && el.hasAttribute('placeholder')) {
        el.placeholder = translations[lang][key];
      } else {
        el.innerText = translations[lang][key];
      }
    }
  });
}

document.addEventListener('DOMContentLoaded', () => {
  applyTranslations();
});
